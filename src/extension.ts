...
async function showWhatsNewMessage(version: string) {
	...
	const result = await vscode.window.showInformationMessage(
		`Better Cron Tasks has been updated to v${version} — check out what's new!`,
		...actions
	);
...
export async function activate(
	...
	if (previousVersion === undefined || currentVersion !== previousVersion) {
		...
		if (previousVersion === undefined) {
			...
			if (
				currentVersion.split(".")[0] > previousVersion.split(".")[0] ||
				(currentVersion.split(".")[0] ===
					previousVersion.split(".")[0] &&
	...
	disposables.push(
		...
		vscode.commands.registerCommand(
			`${CONFIG_KEY}.unregister`,
			unregisterTask
		),
		vscode.commands.registerCommand(
			`${CONFIG_KEY}.showTestMessage`,
			showTestMessage
		)
	);
